...
- Organizer/admin event creation and configuration (events + distances/races + pricing + capacity + website content).
...
- Overloading one model to cover *everything* on day 1 (UltraSignup docs imply many advanced ops knobs—order edits, auto credits, etc.). We should stage intentionally and keep domains separated. (https://help.ultrasignup.com/hc/en-us/articles/30339286849805-Quick-Start-Complete-Your-Event-Setup)
...
Legend for SprintMX column: **v1** (must-have), **v2** (next), **v3** (later) — phased by user value + implementation risk.

| Capability (in scope) | RunSignup | UltraSignup | SprintMX | Notes |
|---|---|---|---|---|
...
#### Edge cases
- Role model alignment: existing global role `external.organizer` grants access to organizer area; org membership controls *which* org’s events a user can manage.
...
- Public:
  - Event directory/search with basic filters (location, date range, distance range, open-only, virtual/in-person). (https://runsignup.com/Races, https://ultrasignup.com/events/search.aspx)
...
- Public can filter events by location/date and open registration.
...
- Also log sensitive *access* events (at minimum): registrations export (CSV) and bulk downloads of registrant data, because these are PII disclosures.
...
- **AddOn** / **AddOnOption** / **AddOnSelection**
  ...
  - Option: `id`, `addOnId`, `label`, `priceDeltaCents`, `maxQtyPerOrder` (default 5)
...
- Registration *—* AddOnOption (via AddOnSelection)
- EventEdition 1—* Waiver; Waiver 1—* WaiverAcceptance
...
- Rules:
  ...
  - Only `Published` + `registrationOpensAt <= now <= registrationClosesAt` counts as “open registration”.
...
3. Create first edition (dates + timezone + location).
4. Save as Draft; system writes AuditLog.
5. Organizer previews public URL (unlisted preview) and then publishes.
...
1. Organizer opens “Website” editor.
2. Chooses enabled sections (Overview, Course, Schedule, FAQ, Media).
3. Edits copy and uploads media (reuse Vercel Blob pattern from profile pictures).
4. Saves; preview updates; AuditLog captures before/after.

### Participant searches and discovers events
1. Participant visits `/events`.
2. Uses location input (reuse our location search API) and filters (date range, distance range, open-only, virtual).
3. Results list shows: event name, date(s), location, key distances, registration status (“open/closed/sold out”).
4. Participant clicks into event page.
...
- Create event series + first edition (orgId + basics + initial dates/location)
- Update edition basics (dates/timezone/location/registration window)
...
- `GET /api/events/search` (filters: q/location/radius/date range/distance range/open-only/virtual/terrain; pagination + sorting)
...
### Public
- `/[locale]/events`:
  - Filters: location (autocomplete), date range, distance range, open-only, virtual.
  - Components: filter panel, results list/cards, pagination.
- `/[locale]/events/[slug]`:
  - Hero + key facts (date/time, location, registration close, capacity status).
  - Distances list + pricing schedule preview.
  - SectionSubnav tabs: Overview, Course, Schedule, FAQ, Photos, Register.
- `/[locale]/events/[slug]/register`:
  - Stepper: Participant → Waiver → (v2) Questions/Add-ons → Review → Payment placeholder.
...
- `/[locale]/dashboard/events/new`:
  - Wizard: Basics → Distances → Capacity → Publish.
- `/[locale]/dashboard/events/[eventId]/*`:
  - Subnav (reuse `SectionSubnav`): Details, Distances, Pricing, Website, Registrations, Policies.
...
- **Authorization gaps (org vs global roles)**: enforce org membership checks in *every* organizer server action; keep a single permissions helper to avoid drift.
...
- How should we represent **division vs gender identity** in SprintMX to match UltraSignup’s “backup division” note? (https://ultrasignup.com/registration_step0.aspx?did=134389&ci=10779327)
