...
- `translateProfileFieldErrors`:
    - Maps generic server field error strings to localized messages with `minAge` handling.
    - The settings profile form uses this to map age-related errors into
      `components.settings.profileForm.errors.minAge`.
...
- [ ] Builds `ProfileUpsertInput` with `buildProfileUpsertPayloadFromForm` or an equivalent mapped
  payload.
- [ ] Uses `translateProfileFieldErrors` for age-related or structured error messages.
...
- Navigation:
    - Side navigation via `SettingsShell` + `SettingsNav` (`components/settings/settings-shell.tsx`,
      `components/settings/settings-nav.tsx`).
    - Horizontal “tabs” via `SettingsSectionSubnav` (
      `components/settings/settings-section-subnav.tsx`) between **Profile** and **Account**.
    - This mirrors the admin users subnav pattern (
      `components/admin/users/users-section-subnav.tsx`).
- Forms:
...
- Admin users:
    ...
    - Horizontal subnav for internal vs self-signup users uses the same Button + Link “tabs” pattern
      as settings.
...
- **Client forms**
    ...
    - Custom mappings:
        - For profile min-age errors, use `translateProfileFieldErrors` before returning
          `fieldErrors`.
...
- **UX**
    ...
    - [ ] In settings/admin views, navigation follows the side-nav + subnav pattern when switching
