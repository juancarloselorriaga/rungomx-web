...
jest.mock('@/lib/rate-limit', () => ({
  cleanupExpiredRateLimits: () => mockCleanup(),
}));

import { GET } from '@/app/api/cron/cleanup-expired-rate-limits/route';
...
describe('Cron Route Handler - cleanup-expired-rate-limits', () => {
  ...
  describe('Authorization', () => {
    it('returns 401 without authorization header in production', async () => {
      ...
      const request = new Request('http://localhost/api/cron/cleanup-expired-rate-limits');
  ...
  describe('Success Response', () => {
    it('returns cleanup result on success', async () => {
      ...
      expect(response.status).toBe(200);
      const body = await response.json();
      expect(body).toEqual({ success: true, deletedCount: 10 });
    });
  });
