...
type ZodIssueMapper = (issue: z.ZodError['issues'][number]) => string | null | undefined;

type ValidateInputOptions = {
  issueMapper?: ZodIssueMapper;
...
export function extractFieldErrors(
  error: z.ZodError,
  issueMapper: ZodIssueMapper = (issue) => issue.message,
): Record<string, string[]> {
  const fieldErrors: Record<string, string[]> = {};

  error.issues.forEach((issue) => {
    const field = issue.path[0]?.toString();
    const mappedMessage = issueMapper(issue);
    if (field) {
      if (!mappedMessage) {
        return;
      }
      ...
      fieldErrors[field].push(mappedMessage);
    }
  });
  ...
  if (!result.success) {
    return {
      ...
      error: {
        ok: false,
        error: 'INVALID_INPUT',
        fieldErrors: extractFieldErrors(result.error, options?.issueMapper),
        message: options?.validationMessage ?? 'Validation failed',
      },
    };
  }
