...
import { getEventLocalScheduleFacts } from '@/lib/events/datetime';
import {
...
type EventAiWizardPromptContext = {
  ...
  activeStepId: EventSetupWizardStepId;
...
const ALLOWED_STEP_IDS = EVENT_SETUP_WIZARD_STEP_IDS;
...
function buildParticipantFacingScheduleDetails(
  ...
  const dateOnlySource = startsAtIsDateOnly
    ? event.startsAt
    : endsAtIsDateOnly
      ? event.endsAt
      : null;
  ...
  return {
    dateLabel: dateOnlySource
      ? formatDateOnlyFactLabel(dateOnlySource, locale)
      : (facts.dateLabel ?? null),
...
function describeGroundingRules(
  ...
  if (event.description?.trim()) {
    knownFacts.push(
      '- A public event description already exists in the snapshot. Reuse facts from it before inventing anything new.',
    );
  }
...
function describeReadinessGuardrails(checklist: EventAiWizardPromptChecklistItem[]): string[] {
  const needsBasics = checklist.some(
    (item) => item.stepId === 'basics' && item.severity !== 'optional',
  );
  const needsDistances = checklist.some(
    (item) => item.stepId === 'distances' && item.severity !== 'optional',
  );
  const needsPricing = checklist.some(
    (item) => item.stepId === 'pricing' && item.severity !== 'optional',
  );
  ...
  if (rules.length === 1) {
    rules.push(
      '- Core setup is present. You may write more fully, but still stay grounded in known facts.',
    );
  } else {
    rules.push(
...
function describeActiveStepDiagnosis(
  ...
  if (
    ...
    (context.activeStepId !== 'basics' && context.activeStepId !== 'pricing')
  ) {
    return [];
  }
...
function describeActiveStep(stepId: EventAiWizardPromptContext['activeStepId']): string[] {
  switch (stepId) {
    case 'basics':
      return [
        'Active step: Basics.',
...
export function buildEventAiWizardSystemPrompt(
  ...
  if (compactMode) {
    ...
    return [
      ...
      ...(describeLocationResolution(context).length > 0
